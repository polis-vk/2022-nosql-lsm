...
public class FileWriter {
    ...
    public void write(BaseEntry<MemorySegment> entry) throws IOException {
        Files.write(dataFilename, ByteBuffer.allocate(Long.BYTES).putLong(entry.key().byteSize()).array(),
                StandardOpenOption.APPEND);
        Files.write(dataFilename, entry.key().toByteArray(), StandardOpenOption.APPEND);
        Files.write(dataFilename, ByteBuffer.allocate(Long.BYTES).putLong(entry.value().byteSize()).array(),
                StandardOpenOption.APPEND);
